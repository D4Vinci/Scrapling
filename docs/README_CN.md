...
A: 使用 `StealthyFetcher`，它会自动处理 Cloudflare Turnstile：
```python
from scrapling.fetchers import StealthyFetcher
...
from scrapling.networking import ProxyRotator

class MySpider(Spider):
    def configure_proxy(self):
        return ProxyRotator.from_list(["proxy1:port", "proxy2:port"])
```
...
| 英文 | 中文 | 说明 |
|------|------|------|
| Fetcher | 获取器 | 用于获取网页内容的类 |
| Session | 会话 | 保持连接状态的上下文管理器 |
| Spider | 爬虫 | 自动化爬取框架 |
| Selector | 选择器 | CSS/XPath 元素选择工具 |
| Adaptive | 自适应 | 自动适应网站变化的特性 |
| Stealthy | 隐秘模式 | 防检测的抓取模式 |
| Proxy | 代理 | 用于隐藏真实 IP 的中介服务 |
| Headless | 无头模式 | 不显示浏览器界面的运行模式 |
| Checkpoint | 检查点 | 用于恢复爬取进度的保存点 |
