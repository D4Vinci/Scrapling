...
The URL can be a WebSocket endpoint (`ws://`/`wss://`), which is what managed browser providers hand out, or the HTTP endpoint of a browser (ex: `http://localhost:9222`).
...
|    pierce_shadow    | Disabled by default. When enabled, open shadow roots are inlined into the returned page content as `<template shadowrootmode="open">` elements, so shadow DOM content becomes queryable with regular selectors. Closed shadow roots can't be read from JavaScript and are left out. |    ✔️    |
