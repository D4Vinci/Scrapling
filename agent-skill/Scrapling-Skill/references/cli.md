...
The Scrapling Interactive Shell is an enhanced IPython-based environment designed specifically for Web Scraping tasks. It provides instant access to all Scrapling features, clever shortcuts, automatic page management, and advanced tools such as curl command conversion.

**Prerequisites:** You should be familiar with [fetcher basics](fetching/choosing.md), [querying elements](parsing/selection.md), [main classes](parsing/main_classes.md), and at least one fetcher ([HTTP requests](fetching/static.md), [dynamic websites](fetching/dynamic.md), or [stealthy fetching](fetching/stealthy.md)).
...
Once launched, you can immediately start scraping — no imports needed:
...
>>> urls = page.css('.titleline>a::attr(href)')
```
...
The `page` and `response` variables are automatically updated with the last fetched page:
...
The `pages` variable keeps track of the last five pages (it's a `Selectors` object):
...
Convert curl commands from browser DevTools to `Fetcher` requests using `uncurl` and `curl2fetcher`.

**Convert curl command to a Request object:**
