...
Everyone is welcome to contribute to Scrapling. 
...
- If you are not a developer, perhaps you would like to help with reviewing the translation of the [documentation](https://github.com/D4Vinci/Scrapling/tree/dev/docs)? We are not aiming to add new languages right now.
...
- **The PR must be made against the [**dev**](https://github.com/D4Vinci/Scrapling/tree/dev) branch of Scrapling. We will reject any PR made against the main branch.**
- **The code should pass all available tests. We use tox with GitHub's CI to run the current tests on all supported Python versions for every code-related commit.**
- **The code should pass all code quality checks like `mypy` and `pyright`. We are using GitHub's CI to enforce code style checks as well.**
...
- Make sure there's already no other PR that solves the issue unless you think your PR is better. In that case, you should state why it's better.
- Try to avoid changing our design choices or open a discussion about it first. A general rule of thumb: too much hand-holding is bad, since most of our users are developers.
...
- There are also the [help wanted](https://github.com/D4Vinci/Scrapling/issues?q=is%3Aissue%20label%3A%22help%20wanted%22%20state%3Aopen) issues, but know that some may require familiarity with the Scrapling codebase first. You can also target any other issue, as long as it isn't tagged `invalid`, `wontfix`, or similar.
...
- We use type hints for better code clarity and [pyright](https://github.com/microsoft/pyright)/[mypy](https://github.com/python/mypy) for static type checking. If your code isn't acceptable to those tools, your PR won't pass the code quality rule.
- We use the conventional commit message format as [here](https://gist.github.com/qoomon/5dfcdf8eec66a051ecd85625518cfd13#types), so for example, we use the following prefixes for commit messages:
   ...
   | `chore:`    | Maintenance tasks        |
    ...
    Then include the change details in the commit message body/description.
